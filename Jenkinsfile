pipeline {
    ...
    stages {
        ...
        stage('Push Docker Image To JCR') {
            steps {
                sh "docker login -u cadmin -p P@ssw0rd http://192.168.5.15:8081/artifactory/docker_jfrog_repo/"
                sh "docker build -f Dockerfile . -t  192.168.5.15:8081/docker_jfrog_repo/${IMAGE_NAME}:${BUILD_NUMBER}"
                sh "docker push 192.168.5.15:8081/docker_jfrog_repo/${IMAGE_NAME}:${BUILD_NUMBER}"
            }
        }
